...
const TherapistDetailScreen = () => {
    ...
    const { therapistId, therapistName, therapistAvatar, therapistBio, therapistExpertise } = route.params;
    const { isDark } = useColorScheme();
    const { therapist, reviews, availability, loading } = useTherapistDetail(therapistId);

    const [isOnline, setIsOnline] = useState(true); // Mock status
    const [isFavorite, setIsFavorite] = useState(false);

    // Default expertise if not provided
    const displayExpertise = therapist?.expertise_areas || therapistExpertise || ['Anxiety', 'Depression', 'Work Stress', 'Relationships'];
    const displayBio = therapist?.bio || therapistBio;
    const displayName = therapist?.full_name || therapistName;
    const displayAvatar = therapist?.avatar_url || therapistAvatar;
    ...
    const renderReviewItem = ({ item }: { item: any }) => (
        <View className="bg-white dark:bg-gray-800 p-4 rounded-2xl mr-4 shadow-sm w-64 border border-gray-100 dark:border-gray-700">
            ...
            <Text className="text-gray-500 dark:text-gray-400 text-xs">- {item.patient?.full_name || 'Anonymous'}</Text>
        </View>
    );
    ...
    return (
        <View className="flex-1 bg-white dark:bg-background-dark">
            ...
            <BottomActionBar
                ...
                onSecondaryPress={() => navigation.navigate('ChatDetail', { otherUserId: therapistId, otherUserName: therapistName })}
                primaryLabel="Book Session"
                onPrimaryPress={() => navigation.navigate('SelectDate', { therapistId, therapistName, therapistAvatar })}
            />
        </View>
    );
};
