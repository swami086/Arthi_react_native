...
import { updateAppointmentStatus } from '../../../api/therapistService';
...
export default function SessionDetailScreen() {
        ...
        const fetchAppointment = async () => {
            try {
                const { data, error } = await supabase
                    .from('appointments')
                    .select('*, profiles:patient_id(full_name, avatar_url), therapist:therapist_id(full_name, avatar_url, bio)')
                    .eq('id', appointmentId)
                    .single();
            ...
            <ScrollView className="px-4" showsVerticalScrollIndicator={false}>
                ...
                <View className="bg-white rounded-[24px] p-4 flex-row items-center mb-4 shadow-sm border border-gray-50">
                    <View className="flex-1">
                        ...
                        <Text className="text-lg font-bold text-[#1a202c]">{therapist?.full_name || 'Dr. Anya Sharma'}</Text>
                ...
                <View className="bg-white rounded-[24px] p-4 flex-row items-center mb-4 shadow-sm border border-gray-50">
                    <View className="flex-1">
                        ...
                        <Text className="text-lg font-bold text-[#1a202c]">{patient?.full_name || 'Ethan Carter'}</Text>
    ...
    const renderLightScribe = () => (
        <View className="flex-1 bg-white">
            ...
            <ScrollView className="px-4" showsVerticalScrollIndicator={false}>
                <Text className="text-2xl font-bold mb-6">Session with {patient?.full_name?.split(' ')[0] || 'Alex'}</Text>
    ...
    const renderDarkScribe = () => (
        <View className="flex-1 bg-[#121f21]">
            ...
            <ScrollView className="px-4" showsVerticalScrollIndicator={false}>
                <Text className="text-2xl font-bold text-white mb-6">Session with {patient?.full_name?.split(' ')[0] || 'Alex'}</Text>
                ...
                <View className="space-y-3 gap-3 mb-8">
                    ...
                    <View className="flex-row items-center gap-4 bg-[#1a2b33] p-4 rounded-xl">
                        ...
                        <View>
                            ...
                            <Text className="text-white font-bold text-base">{patient?.full_name || 'Alex'}</Text>
                        </View>
                    </View>
                    <View className="flex-row items-center gap-4 bg-[#1a2b33] p-4 rounded-xl">
                        <View className="bg-[#2a3d45] p-2 rounded-lg">
