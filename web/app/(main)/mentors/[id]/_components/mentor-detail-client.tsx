...
export default function MentorDetailClient({ mentor, reviews, availability }: MentorDetailClientProps) {
    ...
    const handleFavorite = () => {
        ...
        addBreadcrumb('Mentor favorite toggled', 'mentor.favorite', 'info', { mentorId: mentor.user_id, isFavorite: !isFavorite });
    };

    const handleMessage = () => {
        addBreadcrumb('Navigate to message', 'mentor.message', 'info', { mentorId: mentor.user_id });
        router.push(`/messages?userId=${mentor.user_id}`);
    };

    const handleBookSession = () => {
        addBreadcrumb('Navigate to book session', 'mentor.book', 'info', { mentorId: mentor.user_id });
        router.push(`/appointments/book?mentorId=${mentor.user_id}`);
            ...
            >
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start gap-6 md:gap-10 border-t border-b border-gray-100 dark:border-gray-800 py-4">
                            <div className="space-y-1">
                                <div className="flex items-center gap-1 justify-center md:justify-start">
                                    <span className="text-xl font-black text-gray-900 dark:text-gray-100">
                                        {mentor.rating_average?.toFixed(1) || '5.0'}
                                    </span>
                                    <RatingStars rating={mentor.rating_average || 5} size={14} />
                                </div>
                            ...
                            <div className="space-y-1">
                                <p className="text-xl font-black text-gray-900 dark:text-gray-100">
                                    {mentor.years_of_experience || 1}+ Years
                                </p>
                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                                    Experience
                                </p>
                ...
                <motion.section variants={scaleIn} className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                        {mentor.expertise_areas?.map((area: string, i: number) => (
                            <TagPill
                                key={i}
                                label={area}
