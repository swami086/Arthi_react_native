...
export const ConversationCard: React.FC<ConversationCardProps> = ({
    ...
    return (
        <TouchableOpacity
            onPress={onPress}
            activeOpacity={0.7}
            className={`flex-row items-center p-4 mb-2 rounded-2xl ${unreadCount > 0 ? 'bg-blue-50/50' : 'bg-white'
                }`}
        >
            ...
            <View className="flex-1">
                <View className="flex-row justify-between items-center mb-1">
                    <Text className={`text-base text-gray-900 ${unreadCount > 0 ? 'font-bold' : 'font-semibold'}`}>
                        {name}
                    </Text>
                    <Text className={`text-xs ${unreadCount > 0 ? 'text-primary font-bold' : 'text-gray-400'}`}>
                        {formatDistanceToNow(new Date(timestamp), { addSuffix: true }).replace('about ', '')}
                    </Text>
                </View>
                <View className="flex-row justify-between items-center">
                    <Text
                        className={`text-sm flex-1 mr-4 ${unreadCount > 0 ? 'text-gray-900 font-medium' : 'text-gray-500'}`}
                        numberOfLines={1}
                    >
                        {lastMessage}
                    </Text>
